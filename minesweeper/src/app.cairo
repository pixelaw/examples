...
#[derive(Debug, PartialEq, Serde, Copy, Drop, Introspect, Default)]
pub enum Difficulty {
    #[default]
    None,
    Easy,
...
pub trait IMinesweeperActions<T> {
    ...
    fn init_game(ref self: T, default_params: DefaultParameters, difficulty: Difficulty);
    fn place_mines_randomly(ref self: T, game_position: Position, size: u32, mines_amount: u32);
    fn explode_game(ref self: T, mine_position: Position, game_position: Position);
    fn check_win_condition(ref self: T, game_position: Position);
}
...
pub mod minesweeper_actions {
    ...
    impl ActionsImpl of IMinesweeperActions<ContractState> {
        ...
        fn init_game(
            ...
            let pixel: Pixel = core_world.read_model(position);
            assert!(
                pixel.app.is_zero() || pixel.app == get_contract_address(), "Position occupied",
            );
